...
function GameManager.onPlayerMove(player, direction)
    if GameManager.currentState ~= GameState.PLAYING then return end
...
function GameManager.startGameLoop()
    ...
    GameManager.gameLoop = RunService.Heartbeat:Connect(function()
        ...
        for ghost, ghostData in pairs(GameManager.modules.GhostAI.ghosts) do
            local caughtPlayer = GameManager.modules.GhostAI.updateGhost(ghostData, GameManager.modules.PlayerController)
        ...
        GameManager.modules.PelletManager.checkCollections(
            GameManager.modules.PlayerController,
            GameManager
        )
    end)
end
